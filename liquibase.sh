...
function usage()
{
	...
	for g in $(echo "$output" | egrep -io '^liquibase\:[a-z]+$' | sed -r 's/^liquibase:([a-z]+)$/\1/gi'); do
		# This following command will include the first line which is not mandatory...
...
# Set the default liquibase options and parse command line
LIQUIBASE_OPTS="";
...
start_date=$(date +%s);
mvn $LIQUIBASE_OPTS resources:resources liquibase:$1 -Pdb-migration;
